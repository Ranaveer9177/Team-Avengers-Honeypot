...
Environment Variables (optional):
    ...
    DASHBOARD_PASSWORD    Dashboard password (default: auto-generated HoneyPot@XXXX)
...
$firewallPorts = @(
    @{Port=2222;  Name="Honeypot-SSH";       Desc="Honeypot SSH Service"},
    @{Port=8080;  Name="Honeypot-HTTP";      Desc="Honeypot HTTP Service"},
    @{Port=8443;  Name="Honeypot-HTTPS";     Desc="Honeypot HTTPS Service"},
    @{Port=2121;  Name="Honeypot-FTP";       Desc="Honeypot FTP Service"},
    @{Port=3306;  Name="Honeypot-MySQL";     Desc="Honeypot MySQL Service"},
    @{Port=5001;  Name="Honeypot-Dashboard"; Desc="Honeypot Dashboard"}
)
...
foreach ($rule in $firewallPorts) {
    ...
    if (-not $existing) {
        try {
            New-NetFirewallRule -DisplayName $rule.Name -Direction Inbound -Protocol TCP -LocalPort $rule.Port -Action Allow -Description $rule.Desc -ErrorAction Stop | Out-Null
            Write-Success "Firewall: Opened port $($rule.Port)/tcp ($($rule.Desc))"
        } catch {
            Write-Warning-Custom "Firewall: Could not open port $($rule.Port) (run as Administrator)"
        }
    } else {
        Write-Success "Firewall: Port $($rule.Port)/tcp already configured ($($rule.Name))"
    }
}
Write-Host ""
...
# Password is now auto-generated by app.py — no hardcoded default needed
...
Write-Host "  Password: (auto-generated — check console output above)"
Write-Host ""
