...
def generate_encrypted_password():
    """VULN-019 FIX: Generate a strong password with 12 alphanumeric chars instead of just 4 digits"""
    alphabet = string.ascii_letters + string.digits
    random_suffix = ''.join(secrets.choice(alphabet) for i in range(12))
    return f"honeypot@{random_suffix}"
...
def start_encrypted_honeypot():
    ...
    # VULN-005 FIX: Hash password before saving to config
    salt = os.urandom(16)
    password_hash = hashlib.pbkdf2_hmac(
        'sha256', ssh_password.encode('utf-8'), salt, 100000
    ).hex()
    ...
    try:
        ...
        print(f"  [+] Encrypted password generated: {ssh_password}")
        print(f"  [+] Password hash saved (plaintext NOT stored on disk)")
    ...
    print("\n  🔒 Security:")
    print("     - Password format: honeypot@XXXXXXXXXXXX (12 random alphanumeric chars)")
    print("     - Password is HASHED on disk (config/ssh_password.json)")
